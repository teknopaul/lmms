...
DuckingControllerDialog::~DuckingControllerDialog()
{
...
void DuckingControllerDialog::askUserDefWave()
{
	SampleBuffer * sampleBuffer = dynamic_cast<DuckingController*>(this->model())->
...
void DuckingControllerDialog::contextMenuEvent( QContextMenuEvent * )
{
	/*
...
void DuckingControllerDialog::modelChanged()
{
	m_ducking = castModel<DuckingController>();

	m_baseKnob->setModel( &m_ducking->m_baseModel );
	m_speedKnob->setModel( &m_ducking->m_speedModel );
	m_amountKnob->setModel( &m_ducking->m_amountModel );
	m_phaseKnob->setModel( &m_ducking->m_phaseModel );
	m_waveBtnGrp->setModel( &m_ducking->m_waveModel );
	m_multiplierBtnGrp->setModel( &m_ducking->m_multiplierModel );
}
