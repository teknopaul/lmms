 ...
 * Support timestretching as wella s pitch adjusting via slightly wierd mechanism
 * freq shifting is fast and built into SampleBuffer so to timestretch
 * it freq knob pitches up and thus plays faster, setting timestrech uses
 * RubberBand to pitch down, so freq knob essentially become tempo.
 * RubberBand is slow, grainy and requires the song to stop to take effect.
 * Click the option on and off to recalculate the pitch.
 * This only makes sense in Piano mode, but in automation mode this works like
 * a simple access to a time stretch so its left on.
 *
...
/**
 * In timestrech mode we let SampleBuffer pitch up but we pitch down the actual sample
 * ideally by examly the same amount, this returns that amount.
 * I have no idea why it requires a magic factor
 */
const double Voxpop::getDepitchScale()
{
	const double magicFactor = 2.0;
	return 1.0 - ((m_freqModel.value() - DefaultBaseFreq) / DefaultBaseFreq / magicFactor);
}
...
void Voxpop::timestretchChanged()
{
	if ( m_cueCount > 0 )
	{
		// dont play during reload of samples
		Engine::getSong()->stop();
			...
			if ( DefaultBaseFreq != m_freqModel.value() )
			{
				...
				RubberBandStretcher * rubberBand =
						new RubberBandStretcher(m_sampleBuffer.sampleRate(),
															1,
															RubberBandStretcher::DefaultOptions | RubberBandStretcher::OptionPitchHighQuality,
															1.0,
															getDepitchScale());
